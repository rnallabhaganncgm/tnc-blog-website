...
export default async function ExchangeNews() {
  // Fetch data from server
  const posts: Post[] = await apiClient("/api/posts/category/exchange-news?limit=5", {
    method: "GET",
    cache: "no-store",
  });
  ...
  if (!posts || posts.length === 0) {
    return (
      <div className="py-20 text-center text-gray-500 text-lg">
        No exchange news articles available 😕
      </div>
    );
  }
      ...
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        <div className="relative w-full xl:w-auto">
          ...
          <div className="flex flex-row items-center space-x-1">
            ...
            <h1 className="text-3xl font-medium">Exchange News</h1>
          </div>
        </div>
        <Button
          variant="outline"
...
function FeaturedCard({ post }: { post: Post }) {
  return (
    <div className="border border-[#0000004d] overflow-hidden h-full p-4 flex flex-col justify-between space-y-10">
      <Link
        href={post.slug}
        className="relative block h-[20rem] bg-[#1c1b1f] overflow-hidden rounded-2xl flex-shrink-0"
      >
        <Image
...
function SideCard({ post }: { post: Post }) {
  return (
    <div className="rounded-lg overflow-hidden">
      <Link
        href={post.slug}
        className="relative block h-[11.5rem] bg-[#1c1b1f] overflow-hidden rounded-2xl"
      >
        <Image
          src={post.thumbnail_url || "/placeholder.png"}
          alt={post.post_title}
